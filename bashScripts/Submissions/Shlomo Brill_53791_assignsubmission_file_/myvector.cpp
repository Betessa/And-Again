...
{
	Thing *t = new Thing[n_allocated]; 
	for(int p = 0; p < n_items-1 ; p++){
		t[p] = data[p];  
	}	
	delete[] data; 
	data = t; 
...
{
    return data[i]; //check if this is mwant by reference
}
...
{
	if (i < n_items){    //check if should be <= or <
		return data[i];
	}
	else{
		throw "the element number is out of bounds";
	}
}
