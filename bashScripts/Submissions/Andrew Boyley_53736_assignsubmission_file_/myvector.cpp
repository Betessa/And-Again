...
void MyVector::pop_back() {
  n_items--;
  if (n_items < (n_allocated / 4)) reallocate(n_allocated / 2);
}
 ...
 * @param new_size
 * Reallocate the memory buffer to be "new_size" length, using new
 * Thing[new_size] Copy all items from the old buffer into the new one. Delete
 * the old buffer using delete[]
 */
