...
{
    if(n_allocated == 0){ reallocate(1);}
    if(n_allocated == n_items){ reallocate(2*n_allocated);}
    data[n_items] = t;
    ++n_items;
}
...
{
    if(i >= n_items){ throw std::out_of_range("I was toooooo biiiiiiig!!");}
    else{ return data[i];}
}
...
{
    Thing *tempData = new Thing[new_size];
