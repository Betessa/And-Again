...
data = nullptr; //setting data pointer to nothing 
n_allocated =0; //no space allocated
n_items=0;     //no items 
...
return n_items; //how many things are in the vector
...
return n_allocated; //size of the actual array
...
throw std::out_of_range("You can't remove from an empty array");
}
if(n_items<(1/4)*n_allocated){

reallocate((1/2)*n_allocated);
}
n_items--;
}
 ...
 */
Thing &MyVector::front()  //what is at position 0
{
...
return data+(n_items); //after the last thing!!
...
{
if(i>n_allocated || i<0){
throw std::out_of_range("you can access memory at i");
}
else{
return data[i];
}
...
{
Thing* new_data = new Thing[new_size];//creating a bigger data
