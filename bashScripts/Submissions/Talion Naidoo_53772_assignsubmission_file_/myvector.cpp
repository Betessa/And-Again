...
{   
    if (n_items==n_allocated){
        if (n_allocated==0){
          ...
          Thing* dataTemp = new Thing[2*n_allocated];
...
{    
    ...
    /*Thing* dataTemp = new Thing[n_allocated];
...
Thing &MyVector::at(size_t i)
{
    ...
    if (i<n_items){
        ...
        throw std::out_of_range ("Ha! Bamboozled again.");
    }
}
...
void MyVector::reallocate(size_t new_size)
{
    ...
    Thing* dataTemp = new Thing[new_size];
    for (size_t i=0;i<n_items;i++){
          dataTemp[i] = data[i];
    }
	delete[] data;
