...
MyVector::MyVector()
{
        data = nullptr; // Pointer to memory
        n_items =0; // Number of elements
        n_allocated =  0; //Capacity of the vector
}
...
{
        if(i >= n_items || i < 0) {throw std::out_of_range("Requested index out of bounds.");}
        return data[i];
