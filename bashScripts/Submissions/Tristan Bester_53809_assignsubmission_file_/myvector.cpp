...
{
    size_t totalBuffer = n_allocated;
    size_t usedBuffer = n_items;

    if((usedBuffer + 1) > totalBuffer){
        reallocate(totalBuffer *2);
    }
...
void MyVector::pop_back()
{
    ...
    if(usedBuffer < (0.25*totalBuffer)){
        reallocate(totalBuffer/2);
    }
}
...
{
    if(i <= size_t(n_items)){//cant be neg cause size_t is an unsigned int.
        return data[i];
    }else{
