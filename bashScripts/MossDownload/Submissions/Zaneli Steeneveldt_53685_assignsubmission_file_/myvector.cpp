...
void MyVector::push_back(const Thing &t)
{
	/*if(size() == 0){
		...
		n_items = 1;
		n_allocated = 1;
	}else if(size() == allocated_length()){
		n_allocated = 2 * size();
		reallocate(allocated_length());
		data[size() - 1] = t;
		n_items++;
	}else{
		data[size() - 1] = t;
...
{
	/*if(size() < 1/4 * allocated_length() && size() != 0){
		n_allocated = 1/2 * allocated_length();
		reallocate(allocated_length());
		data[size()-1] = 0;
		n_items--;
	}else{
...
Thing &MyVector::at(size_t i)
{
	if(i < n_items){
		...
		throw std::out_of_range("Request index out of bounds");
	}
}
