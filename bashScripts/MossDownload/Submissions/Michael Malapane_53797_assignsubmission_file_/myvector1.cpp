...
{
if(n_allocated ==0){  // if amount of memory is zero, there is no memory
  reallocate(1);	// allocate memory
  data[n_items] = t;     // this line is within my  function so after checkin the  statements, t will be added and number of items will increase
}

else if(n_items == n_allocated){  // if number of items  = amount of memory
   
	reallocate(2*n_allocated);   // then allocate twice the memory
	data[n_items] = t;     // this line is within my  function so after checkin the  statements, t will be added and number of items will increase
}
else {
	data[n_items] = t;     // this line is within my  function so after checkin the  statements, t will be added and number of items will increase
...
Thing *MyVector::begin()
{
	...
	return data; // characteristics of an array.. will automatically return the 1st element 
}
...
Thing *MyVector::end()
{
	 // even return data , cause data   is an array .. 
	return (data +n_items);
}
