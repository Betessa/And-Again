...
{
    std::cout << "n_items:" << n_items << "\t n_allocated: " << n_allocated << std::endl;
    if (n_allocated == 0 || data == NULL){
        data = new Thing[1];
        n_allocated = 1;
...
{
    // Copy everything but the last item to new array
    Thing* newdata = new Thing[n_allocated];
...
{
    Thing* newdata = new Thing[new_size];
