...
void MyVector::push_back(const Thing &t)
{
    ...
    if(n_items < Thing::last_alloc)
    {
        data[n_items] = t;
    }
    else if(n_items == Thing::last_alloc)
    {
        reallocate(n_allocated);
        Thing::last_alloc = n_allocated *2;
        data[n_items] = t;
    }
}
...
{
    n_items = n_items -1;
    if(n_items <= (0.25)*Thing::last_alloc)
    {
        Thing::last_alloc = Thing::last_alloc *0.5;
    }
