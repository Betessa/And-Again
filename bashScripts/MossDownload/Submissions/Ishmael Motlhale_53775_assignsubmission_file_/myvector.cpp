...
{
	data[n_items-1] == nullptr;
	
	if(n_items < n_allocated/4 ){
		relocate(n_allocated/4);
	}
	n_items--;
	// -- n_items
}
...
void MyVector::reallocate(size_t new_size)
{
	...
	Thing * temp = new Thing[new_size]// or Thing[n_allocated]
	for(size_t i=0;i<n_items;i++){
		temp[i] = data[i];
	}
	delete[] data; // delete[] deletes the whole array
	data = temp;
}
