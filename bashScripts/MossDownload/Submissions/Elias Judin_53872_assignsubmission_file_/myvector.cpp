...
{
    n_allocated = 0;
    n_items = 0; //available space
    data = nullptr; //null pntr will pnt to first element of dynamic arr
}
...
void MyVector::push_back(const Thing &t)
{
    if(n_allocated == 0){
        ...
        reallocate(2*n_allocated); //if buffer (no more space in array) is full, allocate 2x the amount fo space
    }
    data[n_items] = t; //add item to data
    n_items++;
}
