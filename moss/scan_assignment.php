...
define('PLAGIARISM_TEMP_DIR', "{$CFG->tempdir}/plagiarism_moss/");

/**
 * Create a temporary directory for this plugin in $CFG->tempdir/plagiarism_moss/ directory
 */
function plagiarism_moss_create_temp_dir() {
 ...
 */
function plagiarism_moss_get_assignment_dir($assignment, $emptydir = false) {
    $dir = PLAGIARISM_TEMP_DIR . $assignment->cmid . '/';
    if ($emptydir) {
        plagiarism_moss_rrmdir($dir);
        mkdir($dir);
    } else if (!is_dir($dir)) {
        mkdir($dir);
    }
 ...
 */
function plagiarism_moss_get_submitted_files($assignmentcontext) {
 ...
 * @return true
 */
function plagiarism_moss_extract_file($file, $extensions, $location, $user = null, $textfileonly = true) {
    ...
    if ($fileextension == '.zip') {
        $validfile = plagiarism_moss_extract_zip($tempfilepath, $extensions, $location, $user, $textfileonly);
    } else if ($fileextension == '.rar') {
        $validfile = plagiarism_moss_extract_rar($tempfilepath, $extensions, $location, $user, $textfileonly);
    } else {
 ...
 * @return NULL|string
 */
function plagiarism_moss_check_additional_code_structure($decompresseddir) {
    ...
    foreach ($filelist as $file) {
        if (!is_dir($decompresseddir . '/' . $file)) {
            $notalldir = true;
            if (!plagiarism_moss_is_compressed_file($file)) {
...
function process_code_file_archive_format($decompresseddir, array $extensions, $location) {
    ...
    foreach ($files as $file) {
        ...
        if (!plagiarism_moss_is_compressed_file($filefullpath)) {
            debugging("File $file is not a compressed file");
            continue;
        }

        plagiarism_moss_extract_file($filefullpath, $extensions, "$location/ext_$file/");
    }
}
...
function process_code_file_directory_format($decompresseddir, array $extensions, $location) {
    ...
    foreach ($files as $file) {
        ...
        if (is_dir($fullpath)) {
            ...
            if (plagiarism_moss_check_extension($file, $extensions)) {
 ...
 * The directory is $CFG->tempdir/plagiarism_moss/student_id/files
 * @param Object $assignment The record object of setting for the assignment
 ...
 */
function plagiarism_moss_extract_assignment($assignment) {
    global $DB;

    echo "[".date(DATE_RFC822)."] ".get_string('extract', 'plagiarism_moss')."\n";
    // Make a subdir for this assignment in the plugin subdir and emptying it.
    $tempsubmissiondir = plagiarism_moss_get_assignment_dir($assignment, true);
    ...
    $filerecords = plagiarism_moss_get_submitted_files($context);

    if (count($filerecords) < 2) {
        return NOT_SUFFICIENT_SUBMISSION;
    }

    $extensions = plagiarism_moss_get_file_extension($assignment->language);
    $validsubmissions = 0;
    ...
    foreach ($filerecords as $file) {
        ...
        if (plagiarism_moss_is_compressed_file($file->get_filename())) { // Decompress file.
            $validfile = plagiarism_moss_extract_file($file, $extensions, $userdir, $student);
        } else if (plagiarism_moss_check_extension($file->get_filename(), $extensions)) {
    ...
    $fs = get_file_storage();
    $additionalcodefiles = $fs->get_area_files($context->id, 'plagiarism_moss', 'codeseeding', $assignment->id, '', false);

    $count = 1;
    foreach ($additionalcodefiles as $codefile) {

        $filename = $codefile->get_filename();
        if (!plagiarism_moss_is_compressed_file($filename)) {
        ...
        plagiarism_moss_extract_file($codefile, null, $tempdir, null, false);
        $codefiletype = plagiarism_moss_check_additional_code_structure($tempdir);
        switch ($codefiletype) {
        ...
        plagiarism_moss_rrmdir($tempdir);
    }
 ...
 */
function plagiarism_moss_submit_assignment($assignment, $tool, $scaninfo) {
    global $DB;
    ...
    $scaninfo->progress = 0;
    $DB->update_record('plagiarism_moss_' . $toolname, $scaninfo);

    mtrace("[".date(DATE_RFC822)."] Start sending to $toolname");
    $tempsubmissiondir = plagiarism_moss_get_assignment_dir($assignment);
    ...
    // Note that scan_info is the object containing the corresponding record in table plagiarism_moss_jplag.
    return $scaninfo;
}
 ...
 * @return $scaninfo The updated scan_info object.
 */
function plagiarism_moss_check_scanning_status($assignment, $tool, $scaninfo) {
    global $DB;
    ...
    if ($scaninfo->status == 'scanning') {
        ...
        $DB->update_record('plagiarism_moss_' . $tool->get_name(), $scaninfo);
    }
    return $scaninfo;
}
 ...
 * after calling plagiarism_moss_check_scanning_status function
 *
 * @param Object $assignment The record object of setting for the assignment
 * @param Object $tool The tool object (either of type moss_tool or jplag_tool)
 * @param Object $scaninfo The status record object of that tool
 ...
 */
function plagiarism_moss_download_result($assignment, $tool, $scaninfo) {
    global $DB;
 ...
 */
function plagiarism_moss_is_uploaded($assignment) {
    ...
    foreach ($detectiontools as $toolname => $toolinfo) {
        ...
        $scaninfo = $DB->get_record('plagiarism_moss_' . $toolname, array(
 ...
 * @return void
 */
function plagiarism_moss_scan_assignment($assignment, $waitforresult = true, $notificationmail = false) {
    ...
    if (!plagiarism_moss_is_uploaded($assignment)) {
        $extractresult = plagiarism_moss_extract_assignment($assignment);
        if ($extractresult === true) {
            ...
            $message = get_string('invalid_file_type', 'plagiarism_moss').implode(', ' , plagiarism_moss_get_file_extension($assignment->language));

            foreach ($detectiontools as $toolname => $tool) {
                if (!$assignment->$toolname) { // This detector is not selected.
                    continue;
                }
                $scaninfo = $DB->get_record('plagiarism_moss_' . $toolname, array('settingid' => $assignment->id));
    ...
    foreach ($detectiontools as $toolname => $tool) {
        ...
        $DB->update_record('plagiarism_moss_' . $toolname, $scaninfo);
        $links[] = "$CFG->wwwroot/plagiarism/moss/scan_after_extract.php?"
    ...
    $assignment = $DB->get_record('plagiarism_moss', array('id' => $assignment->id));
    $assignment->latestscan = time();
    $DB->update_record('plagiarism_moss', $assignment);

    plagiarism_moss_curl_download($links, $logfiles);
...
function scan_after_extract_assignment($assignment, $toolname, $waittodownload = true, $notificationmail = false) {
    ...
    if ($scaninfo->status == 'pending' || $scaninfo->status == 'error') {
        ...
        $scaninfo = plagiarism_moss_submit_assignment($assignment, $toolclass, $scaninfo);
    }

    plagiarism_moss_check_scanning_status($assignment, $toolclass, $scaninfo);
    ...
    if ($scaninfo->status == 'done') {
        ...
        plagiarism_moss_download_result($assignment, $toolclass, $scaninfo);
 ...
 */
function plagiarism_moss_handle_shutdown() {
    global $DB, $CFG, $detectiontools;
    ...
    if ($processinginfo && $error && ($error['type'] == E_ERROR)) {
        ...
        $assignment = $DB->get_record('plagiarism_moss', array(
            'cmid' => $cmid
        ));
        ...
        foreach ($tools as $tool) {
            if ($assignment->$tool) {
                $scaninfo = $DB->get_record('plagiarism_moss_' . $tool, array(
                    'settingid' => $assignment->id
                ));
                $scaninfo->status = 'error';
                $message = get_string('unexpected_error', 'plagiarism_moss');
                if ($stage == 'extract') {
                    $message = get_string('unexpected_error_extract', 'plagiarism_moss');
                } else if ($scaninfo->status == 'uploading') {
                    $message = get_string('unexpected_error_upload', 'plagiarism_moss');
                } else if ($scaninfo->status == 'downloading') {
                    $message = get_string('unexpected_error_download', 'plagiarism_moss');
                }
                $scaninfo->message = $message;
                $DB->update_record('plagiarism_moss_' . $tool, $scaninfo);
            }
        }
    }
